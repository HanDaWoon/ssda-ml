from itertools import chain # TODO: 이거 뭐임?
...
from train import setup_language_dependent, setup_data, setup_cv_dset_loader, get_dset_loader, get_val_dset_loader
...
class Evaluator:
    ...
    @torch_eval
    def inference_only(self, gen, style_avails, target_fonts, target_chars, n_max_match=3, compare_inputs=False):
        """ Inference for Custom Input Font """
        # infer
        ...
        print("target_fonts : ", end="")
        pprint(target_fonts)
        print("target_chars : ", end="")
        pprint(target_chars)
        print("style_avails : ", end="")
        pprint(style_avails)
        print("content_font : ", end="")
        pprint(self.content_font)
        ...
        compareable_grid = utils.make_comparable_grid(*compare_batches, nrow=nrow)
        
        print(compareable_grid.shape, "comparable_grid.shape") 
        return compareable_grid    
...
def main():
    ...
    logger = Logger.get() # TODO : Log 구조 파악하기.
    
    parser = argparse.ArgumentParser("Get-Sample")
    ...
    parser.add_argument(
        ...
        help="eval (default) / cv-save / user-study / user-study-save. "
             ...
             "`inference` inference from hdf5 font data to visualize"
    )
    ...
    content_font, n_comp_types, n_comps = setup_language_dependent(cfg) # 언어 종속적인 특성들 가져옴.
    ...
    # setup data
    hdf5_data, meta = setup_data(cfg, transform) # hdf5형태의 data를 준비함.
    ...
    if args.mode == "inference":
        save_dir = Path(args.img_dir) / "inference_images_{}".format(step)
        utils.rm(save_dir)
        # 지금 내가 구현해야할 요구 사항은 
        # 우리가 어떤 글자를 넣는지를 알고, 해당 글자를 이용해서 encoding을 진행한 다음에
        # decoding으로 sample image를 출력하는 것.
        # 내가 공부해야하는 건 kor_dataset.py의 MATargetFirstDataset, MAStyleFirstDataset
        custom_dataloader = get_val_dset_loader(
